...
function rechercheSalle($key)
	{
		$sql = "SELECT * FROM salle WHERE num LIKE '%$key%' OR categorie LIKE '%$key%' OR coach LIKE '%$key%'";
		$db = config::getConnexion() ;
		try {
			$liste = $db->query($sql);
			return $liste;
	...
	function afficherDESC($num)
     {
    $sql="SELECT * FROM salle ORDER BY num DESC";
    $db = config::getConnexion();
    return ($db->query($sql));
    
     }

   function afficherASC($num)
   {
